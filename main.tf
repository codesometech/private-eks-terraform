...
module "eks_blueprints" {
  ...
  cluster_endpoint_public_access  = false # if true, Your cluster API server is accessible from the internet. You can, optionally, limit the CIDR blocks that can access the public endpoint.
  ...
  managed_node_groups = {
    ...
    #---------------------------------------------------------------
    # Note: This example only uses ON_DEMAND node group for both Dask Scheduler and Workers.
    #   If you want to leverage SPOT nodes for workers then create ON_DEMAND node group for placing your driver pods and SPOT nodegroup for executors.
    #   Use NodeSelectors to place your driver/executor pods with the help of Pod Templates.
    #---------------------------------------------------------------
    mng2 = {
      node_group_name = "workloads-node-grp"
      subnet_ids      = [element(var.private_subnet_ids, 0)] # Single AZ node group for workloads
      ...
      k8s_labels = {
        ...
        Zone          = "test"
        WorkerType    = "ON_DEMAND"
        NodeGroupType = "workloads"
      }

      # Checkout the docs for more details on node-template labels https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/FAQ.md#how-can-i-scale-a-node-group-to-0
      additional_tags = {
        Name                                                             = "workloads-node-grp"
        subnet_type                                                      = "private"
        "k8s.io/cluster-autoscaler/node-template/label/arch"             = "x86"
        "k8s.io/cluster-autoscaler/node-template/label/kubernetes.io/os" = "linux"
        "k8s.io/cluster-autoscaler/node-template/label/noderole"         = "workloads"
        "k8s.io/cluster-autoscaler/node-template/label/node-lifecycle"   = "on-demand"
        "k8s.io/cluster-autoscaler/experiments"                          = "owned"
        "k8s.io/cluster-autoscaler/enabled"                              = "true"
      }
...
resource "null_resource" "eks_cluster_context" {
